...
class Compressor {
 ...
 public:
  Compressor(int level = Z_DEFAULT_COMPRESSION,
      ...
      : M_out_buffer(NULL), M_out_size(0), M_out_avail(0) {
  ...
  int reset() { return deflateReset(&M_stream); }
  ...
  int compress(const char* in_buffer, int in_size, int z_flush = Z_NO_FLUSH) {
    if (M_out_buffer == NULL) {
      ...
      if (M_out_buffer == NULL) return Z_MEM_ERROR;
  ...
  int getOutput(char*& out, int& size, bool detach = false) {
    ...
    if (detach) {
      ...
      if (out == NULL) return Z_MEM_ERROR;
...
class Decompressor {
 ...
 public:
  Decompressor() : M_out_buffer(NULL), M_out_size(0), M_out_avail(0) {
    M_stream.zalloc = Z_NULL;
    M_stream.zfree = Z_NULL;
    M_stream.opaque = NULL;
  ...
  int decompress(const char* in_buffer, int in_size, int z_flush = Z_NO_FLUSH) {
    if (M_out_buffer == NULL) {
